...
@interface ProTheDetailViewController ()
{
    int smgSelect;
    ...
    IBOutlet UILabel *lblSoThe;
    IBOutlet UILabel *lblTrangThai;
    IBOutlet UILabel *lblTenChuThe;
    IBOutlet UILabel *lblSoTaiKhoanThanhToan;
    IBOutlet UILabel *lblSoDu;
    IBOutlet UILabel *lblTienTe;
    IBOutlet UILabel *lblRM;
    IBOutlet UILabel *lblChiNhanhPhatHanh;
    DTOProWay4DetailProcess *dtoProWay4DetailProcess;
}
...
@implementation ProTheDetailViewController
...
- (void)viewDidLoad {
    ...
    _fullNameLB.text = TITLE_PRODUCT_DETAIL_THE;
    [self updateInterFaceWithOption:smgSelect];

    dtoProWay4DetailProcess = [DTOProWay4DetailProcess new];

    if (_dtoProductDetailObject) {
        NSMutableArray *resultArr = [dtoProWay4DetailProcess filterProductDetailWithClientId:_dtoProductDetailObject.clientId WithProductCode:_dtoProductDetailObject.productCode];
        if (resultArr.count>0) {
            ...
            [lblTrangThai setText:[proDetailDic valueForKey:@"STATUS"]];
            [lblSoThe setText:[proDetailDic valueForKey:@"CARD_NUMBER"]];
            [lblTrangThai setText:[proDetailDic valueForKey:@"CURRENCY"]];
            [lblTenChuThe setText:[proDetailDic valueForKey:@"CARD_HOLDER_NAME"]];
            [lblSoTaiKhoanThanhToan setText:[proDetailDic valueForKey:@"CURRENT_ACCOUNT"]];

            [lblSoDu setText:[proDetailDic valueForKey:@"BALANCE"]];
            [lblTienTe setText:[proDetailDic valueForKey:@""]];
