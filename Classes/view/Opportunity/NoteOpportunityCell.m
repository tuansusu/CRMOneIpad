...
@implementation NoteOpportunityCell
...
+(NoteOpportunityCell*) initNibCell{
    
    NSArray *topLevelObjects = [[NSBundle mainBundle] loadNibNamed:@"NoteOpportunityCell" owner:nil options:nil];
    
    for(id curentObject in topLevelObjects)
    {
        if([curentObject isKindOfClass:[NoteOpportunityCell class]])
        {
            return (NoteOpportunityCell *) curentObject;
