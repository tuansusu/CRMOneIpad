...
static const CGFloat kGridViewDefaultRowHeight = 50;
...
- (void)layoutSubviews
{
    ...
    for(int row = 0; row < _numberOfRows; row++)
    {
        ...
        for(int column = 0; column < cols; column++)
        {
            ...
            if([self.delegate respondsToSelector:@selector(gridView:didSelectCellAtIndexPath:atCell:)])
            {
                UILongPressGestureRecognizer *tap = [[UILongPressGestureRecognizer alloc] initWithCallback:^(id sender) {
                    [self.delegate gridView:self didSelectCellAtIndexPath:indexPath atCell:cell];
                }];
                //tap.minimumPressDuration = 1.0;
                
                cell.btnTapToCell.gestureRecognizers = [NSArray arrayWithObjects:tap, nil];
