...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i=0; i<[lineBezierPath count]; i++) 
    {
        ...
        if (i>=lineColorArray.count) {
            _color = LINE_LIMIT_COLOR_1;
        }else{
        MIMColorClass *c=[lineColorArray objectAtIndex:i];
        
        _color=[[UIColor alloc]initWithRed:c.red green:c.green blue:c.blue alpha:c.alpha];
        }
...
-(void)_drawAnchorPoints
{
    ...
    for (int index=0; index<[lineBezierPath count]; index++)
    {
        ...
        if (index>=lineColorArray.count) {
            c= MIMC_LINE1;
        }else{
            c=[lineColorArray objectAtIndex:index];
        }
        ...
        if([[yValElements objectAtIndex:index] isKindOfClass:[NSString class]]||[[yValElements objectAtIndex:index] isKindOfClass:[NSNumber class]])
        {
            for (int l=0; l<[yValElements count]; l++)
            {
                ...
                //anchor.anchorTag=l;(tuannv edit 25/11)
                anchor.anchorTag=index * [yValElements count] + l;;
                anchor.tag = index * [yValElements count] + l; //index * yvalueArray.cou + l
...
-(void)displayAnchorInfo:(NSInteger)tagID At:(CGPoint)point
{
    ...
    long long anchorY=[[[yValElements objectAtIndex:indexRow] objectAtIndex:indexColumn] longLongValue];
