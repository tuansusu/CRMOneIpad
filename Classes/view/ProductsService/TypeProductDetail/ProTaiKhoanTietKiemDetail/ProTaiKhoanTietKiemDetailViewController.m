...
- (void)viewDidLoad {
    ...
    _fullNameLB.text = TITLE_PRODUCT_DETAIL_TAI_KHOAN_TIET_KIEM;
    [self updateInterFaceWithOption:smgSelect];

    dtoProTietKiemDetailProcess = [DTOProTietKiemDetailProcess new];

    if (_dtoProductDetailObject) {
        NSMutableArray *resultArr = [dtoProTietKiemDetailProcess filterProductDetailWithClientId:_dtoProductDetailObject.clientId WithProductCode:_dtoProductDetailObject.productCode];
        if (resultArr.count>0) {
            ...
            [lblKyHan setText:[proDetailDic valueForKey:@"TERM_DAY"]];
            [lblLaiDuChi setText:[proDetailDic valueForKey:@"EXPIRED_DATE"]];
