...
@synthesize animateLine;
@synthesize margin;
@synthesize hideInfoWindow;
@synthesize perTileScale;
@synthesize arrayTechTarget;
...
-(BOOL)initAndWarnings
{
    ...
    //NSLog(@"scaley = %f",_scalingY);
    pixelsPerTile=_tileHeight/_scalingY;
    //NSLog(@"_tileHeight = %f",_tileHeight);
    //
//    if ( 100000.0<pixelsPerTile<= 500000) {
//        pixelsPerTile = pixelsPerTile/1000.0;
//    }
//    else if (pixelsPerTile >500000)
//    {
//        pixelsPerTile = pixelsPerTile/10000.0;
//    }
    if (perTileScale >1) {
        pixelsPerTile = pixelsPerTile/perTileScale;
    }
    
   // NSLog(@"pixelsPerTile = %f",pixelsPerTile);
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i=0; i<[myPathArray count]; i++) 
    {
        if (lineColorA.count-1<i) {
            
            UIColor *_color=[[UIColor alloc]initWithRed:0 green:0 blue:0 alpha:1];
            [_color setStroke];
        ...
        else
        {
            for (int k=index; k<index+1; k++) 
            {
                ...
                {   
                    ...
                    //tuannv comment
                   // anchor.anchorTag = l;
                    anchor.anchorTag=index * [yArray_ count] + l;;
                    anchor.tag = index * [yArray_ count] + l; //index * yvalueArray.cou + l
                    anchor.delegate=self;
                    [self addSubview:anchor];
                    [anchor drawAnchor];
                    
                    
                    if ((self.tag == TAG_Graph_TechTarget_Date || self.tag == TAG_Graph_TechTarget_Month) && index>2 && l == (yArray_.count-1)) {
                        [self displayAnchorInfoText:l At:anchor.center withIndex:index];
                    }
...
-(void)displayAnchorInfoText:(NSInteger)tagID At:(CGPoint)point withIndex : (NSInteger) inputIndex
{
    
    //displayAnchorInfo
    //NSLog(@"displayAnchorInfo =%d -  %f - %f",inputIndex, point.x, point.y);
...
//    }
    AnchorInfo *aInfo;
    NSDictionary *dic = [arrayTechTarget objectAtIndex:inputIndex-3];
//    NSString *keyTechTargetName = [dic valueForKey:Key_TechTarget_targetName];
//    
//   // keyTechTargetName = [NSString stringWithFormat:@"%@(%@ - %@)",keyTechTargetName,[dic valueForKey:Key_TechTarget_lowValue],[dic valueForKey:Key_TechTarget_highValue]];
//   
//    long lowerValue = 0; //[[dic valueForKey:Key_TechTarget_lowValue] longValue];
//    long hightValue = 0;//[[dic valueForKey:Key_TechTarget_highValue] longValue];
//    
//    if ([[dic valueForKey:Key_TechTarget_lowValue] isEqual:@""]!=YES) {
//       lowerValue= [[dic valueForKey:Key_TechTarget_lowValue] longValue];
//    }
//    
//    if ([[dic valueForKey:Key_TechTarget_highValue] isEqual:@""]!=YES) {
//        hightValue= [[dic valueForKey:Key_TechTarget_highValue] longValue];
//    }else{
//        //hightValue = lowerValue+10;
//    }
//    
//    if (lowerValue<hightValue) {
//        
//        //NSLog(@"%@ - lower", keyTechTargetName);
//        aInfo=[[AnchorInfo alloc]initWithFrame:CGRectMake(point.x+10, point.y-5, 320, 20)];
//        keyTechTargetName = [NSString stringWithFormat:@"%@(%@ -)",keyTechTargetName,[dic valueForKey:Key_TechTarget_highValue]];
//    }
//    
//    if (lowerValue == hightValue) {
//        //NSLog(@"%@ - equal", keyTechTargetName);
//        aInfo=[[AnchorInfo alloc]initWithFrame:CGRectMake(point.x+10, point.y-15, 320, 20)];
//        keyTechTargetName = [NSString stringWithFormat:@"%@(%@)",keyTechTargetName,[dic valueForKey:Key_TechTarget_lowValue]];
//    }
//    
//    if (lowerValue>hightValue) {
//        //NSLog(@"%@ - hight", keyTechTargetName);
//        aInfo=[[AnchorInfo alloc]initWithFrame:CGRectMake(point.x+10, point.y-50, 320, 20)];
//        keyTechTargetName = [NSString stringWithFormat:@"%@(%@ +)",keyTechTargetName,[dic valueForKey:Key_TechTarget_lowValue]];
//    }
    ...
    //[aInfo setInfoString:keyTechTargetName];
...
-(void)displayAnchorInfo:(NSInteger)tagID At:(CGPoint)point
{
    ...
    //NSLog(@"displayAnchorInfo = %d", tagID);
    int count = [_xTitles count];
    int indexRow=tagID/count;
    int indexColumn = tagID%count;
    //NSLog(@"tag = %d row = %d column = %d", tagID, indexRow, indexColumn);
   // return;
    //Find if the AnchorInfo at that tag is already displaying
    for (id view in self.subviews) {
        if([view isKindOfClass:[AnchorInfo class]])
